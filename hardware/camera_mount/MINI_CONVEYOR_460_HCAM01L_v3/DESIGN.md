# HARDWARE-CAD-002 / 003 설계 계약

현재 보완 작업은 [HARDWARE-CAD-003 Optical Clearance](OPTICAL_VALIDATION.md)다. CAD-002의 기계적 검증은 아래 이력으로 보존하며, 현재 광학 검증 상태는 `validation/stl_validation.json`의 `optical_fov`를 따른다.
...
- Deck은 고정 직각 shelf + 두 gusset이다. CAD-003에서 Camera Y를62→255 mm로 옮겨 Portal을 Conveyor 중앙보다 뒤쪽에 둔다. 두 gusset을 함께 연장하고 뒤쪽 높이를 늘린다. 88.2×38.2 pocket, contact plane 기준 깊이2.5, 네 contact land, 낮은 Stop을 보존한다. Ø30은 Deck top 기준 flare 생성 지름이며 아래쪽은 약Ø49.53이다. Pocket이 윗부분을 이미 비우므로 실제 Deck top 개구는 pocket의88.2×38.2이고, report는 물리적 단면과 생성 지름을 구분한다.
- Camera ring22/offset0은 PHOTO_ESTIMATE, 돌출4 mm와 pupil 지지면+3 mm는 PROVISIONAL이다. FOV90°/70°와 측면 여유5 mm는 가설 설계 조건이며 실제 카메라 FOV 검증이 아니다.
- Cable은 위/뒤→Crossbar 뒤→오른쪽 기둥 뒤로 고정한다. Deck의 Cable tie point와 Velcro 아래 경로를 lens 뒤쪽으로 옮겼다. 10 mm Velcro reference를 사용하며 실제 stand/케이블 출구/최소 굽힘 반경은 실측한다.
